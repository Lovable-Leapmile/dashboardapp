...
const TrayReady = () => {
  ...
  const fetchTasksData = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://amsstores1.leapmile.com/robotmanager/is_tray_ready", {
        method: "GET",
        headers: {
          Authorization:
      ...
      if (response.status === 404 && data.message === "no records found") {
        console.log("No tray ready tasks found");
        setTotalCount(0);
        setRowData([]);
        return;
      }
      ...
      console.log("Fetched tray ready tasks:", data?.records?.length);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#fafafa" }}>
      <AppHeader selectedTab="" isTasksPage={true} activeTaskTab="Tray Ready" />
